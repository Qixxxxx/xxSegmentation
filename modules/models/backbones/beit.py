...
class BEiTAttention(nn.Module):
    def __init__(self, embed_dims, num_heads, window_size, bias='qv_bias', qk_scale=None, attn_drop_rate=0.,
                 proj_drop_rate=0.):
...
class BEiTTransformerEncoderLayer(VisionTransformerEncoderLayer):
    def __init__(self, embed_dims, num_heads, feedforward_channels, attn_drop_rate=0., drop_path_rate=0., num_fcs=2,
                 bias='qv_bias',
                 act_cfg=None, norm_cfg=None, window_size=None, attn_cfg=dict(), ffn_cfg=dict(add_identity=False),
                 init_values=None):
        super(BEiTTransformerEncoderLayer, self).__init__(
            embed_dims=embed_dims, num_heads=num_heads, feedforward_channels=feedforward_channels,
            attn_drop_rate=attn_drop_rate,
            drop_path_rate=0., drop_rate=0., num_fcs=num_fcs, qkv_bias=bias, act_cfg=act_cfg, norm_cfg=norm_cfg,
            attn_cfg=dict(),
            ffn_cfg=ffn_cfg
        )
...
class BEiT(nn.Module):
    def __init__(self, structure_type, img_size=(640, 640), patch_size=16, in_channels=3, embed_dims=768, num_layers=12,
                 num_heads=12, mlp_ratio=4,
                 out_indices=(3, 5, 7, 11), qv_bias=True, attn_drop_rate=0., drop_path_rate=0.1,
                 norm_cfg={'type': 'LayerNorm', 'eps': 1e-6},
                 act_cfg={'type': 'GELU'}, patch_norm=False, final_norm=False, num_fcs=2, init_values=0.1,
    ...
    def buildlayers(self):
        ...
        for i in range(self.num_layers):
            self.layers.append(BEiTTransformerEncoderLayer(
                ...
                attn_drop_rate=self.attn_drop_rate, drop_path_rate=dpr[i], num_fcs=self.num_fcs,
        ...
        for key in all_keys:
            ...
            if 'relative_position_bias_table' in key:
                ...
                if src_size != dst_size:
                    ...
                    new_rel_pos_bias = self.geometricsequenceinterpolation(src_size, dst_size, rel_pos_bias,
                                                                           num_attn_heads)
